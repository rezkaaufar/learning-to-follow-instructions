...
merged = list(itertools.chain(*command_list))
change_dict = {}
merged += ["at", "to", "tile"]
for i in range(len(merged)):
  change_dict[merged[i]] = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
ro_list = [4,1,3,2,0]
ro = True

def corrupt_data(path):
  inputs = []
  targets = []
  instrs = []
  with open(path, "r") as f:
    for line in f:
      ...
      sent_corrupted = ""
      for word in words:
        sent_corrupted += change_dict[word] + " "
