...
model_name = "Seq2Conv_50000_nvl_" + which_data + "_same_seeds_" + model_name
...
encoder = Encoder.EncoderWord(dataset.n_words, args.hidden_size, n_layers=args.layers_lstm)
enc_optimizer = torch.optim.Adam(encoder.parameters(), lr=lr)
optimizer = torch.optim.Adam(decoder.parameters(), lr=lr)
criterion = nn.NLLLoss()
decoder.cuda()
...
for epoch in range(1, n_epochs + 1):
  ...
  if acc_val_seq > cur_best:
    cur_best = acc_val_seq
    print("Writing models at epoch {}".format(epoch))
    with open(dirs + "/models/seq2conv/" + "Encoder_" + model_name + ".tar", 'wb') as ckpt:
      torch.save(encoder, ckpt)
    with open(dirs + "/models/seq2conv/" + "Decoder_" + model_name + ".tar", 'wb') as ckpt:
      torch.save(decoder, ckpt)
    torch.save(decoder.state_dict(),
               dirs + '/models/seq2conv/Params_Decoder_' + model_name + '.tar')
    torch.save(encoder.state_dict(),
               dirs + '/models/seq2conv/Params_Encoder_' + model_name + '.tar')
  # accs.append(acc_seq)
  # accs_tr.append(acc_tr_seq)
...
writer.export_scalars_to_json(dirs + "/json/seq2conv/" + model_name + ".json")
writer.close()
df.to_json(dirs + '/json/seq2conv/'+ model_name + '.pd.json')
